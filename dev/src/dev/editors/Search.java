 ...
 * 修改人：       rxy
 * 修改时间：   2013.11.27
 * 修改内容：    修改了createPartControl方法中， 设置searchByNameRadioButton文本的逻辑，
 *         若editorId等于RetCodeEditor.ID，则设置该按钮的文本为“值”，否则为“名称”。
 */
...
/**
 * 2013.11.7 1.修改Search类，在“查询”按钮的响应中，增加判断， 若在查找到指定的记录后，若要在当前编辑器中显示，且当前编辑器为dirty，
 * 则询问用户是否先保存当前修改或者取消，若是，则先保存当前的修改， 然后显示查询到的记录，若否，则不保存当前修改，直接显示查询到的记录，
 * 若取消，则什么也不做，对“查询”按钮的响应就此返回。 2.修改Search类，在查找到指定的记录后，将对setSaveButtonEnable方法的调用，
 * 替换为对setDirty方法的调用。 2013.11.1 该类实现了在编辑中显示查询所涉及的控件， 以及完成查询功能的方法，通过在编辑器中使用该类的实例，
 * 即可以完成编辑器所需要的查询功能， 从而不需要在编辑器定义查询所涉及的控件，以及实现查询方法。 使用方法： 1.编辑器需要实现ISearch接口；
 * 2.给编辑器类新增一个私有的Search类型的成员变量；（以下假定该成员变量的名字为search）
 * 3.在编辑器类的createPartControl()方法中依次调用： search = new Search();
 * search.setUpProject(); search.setEditorId(); search.setEditor();
 * search.createPartControl(); 注意：需要将编辑器中所使用的map中的诸如“serverid”的键改为“id”，
...
public class Search {
	...
	public void createPartControl(Group group) {
		...
		searchTypeLabel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER,
				false, false, 1, 1));
		...
		if (editorId.equals(RetCodeEditor.ID)) {
			searchByNameRadioButton.setText("值");
		} else {
			...
			public void widgetSelected(SelectionEvent e) {
						...
						if (searchByIdRadioButton.getSelection()) {
							if (searchEditor.getTargetId().equals(
								...
								PlatformUI
										...
										.getActivePage()
										.bringToTop(
												(IWorkbenchPart) searchEditor);
								return;
							}
						} else {
							if (searchEditor.getTargetName().equals(
									searchText.getText())) {
								PlatformUI
										.getWorkbench()
										.getActiveWorkbenchWindow()
				...
				try {
					...
					case ServerEditor.ID:
						...
						if (searchByIdRadioButton.getSelection()) {
							map = editorsServerServiceImpl
									.queryServerByIdOrName(
											searchText.getText(), 0,
											editor.getPs());
						} else {
							map = editorsServerServiceImpl
									.queryServerByIdOrName(
											searchText.getText(), 1,
											editor.getPs());
						}
						break;
					case TradeEditor.ID:
						editorType = "交易";
						EditorsDaoServiceImpl editorsDaoServiceImpl1 = new EditorsDaoServiceImpl();
						if (searchByIdRadioButton.getSelection()) {
							map = (HashMap<String, String>) editorsDaoServiceImpl1
									.queryTradeByIdOrName(searchText.getText(),
											"", editor.getPs());
						} else {
							map = (HashMap<String, String>) editorsDaoServiceImpl1
									.queryTradeByIdOrName("",
											searchText.getText(),
											editor.getPs());
						}
						break;
					case AopDllEditor.ID:
						editorType = "动态库";
						EditorAopDllServiceImpl editorAopDllServiceImpl = new EditorAopDllServiceImpl();
						if (searchByIdRadioButton.getSelection()) {
							map = (HashMap<String, String>) editorAopDllServiceImpl
									.queryAopDllByIdOrName(
											searchText.getText(), "",
											editor.getPs());
						} else {
							map = (HashMap<String, String>) editorAopDllServiceImpl
					...
					case AopEditor.ID:
						...
						if (searchByIdRadioButton.getSelection()) {
							map = (HashMap<String, String>) editorAopServiceImpl
									.queryAopByIdOrName(searchText.getText(),
											"", editor.getPs());
						} else {
							map = (HashMap<String, String>) editorAopServiceImpl
									.queryAopByIdOrName("",
						...
						break;
					case DataItemEditor.ID:
						editorType = "数据项";
						EditorDataitemServiceImpl editorDataitemServiceImpl = new EditorDataitemServiceImpl();
						if (searchByIdRadioButton.getSelection()) {
							map = (HashMap<String, String>) editorDataitemServiceImpl
									.queryDataitemByIdOrName(
											searchText.getText(), "",
											editor.getPs());
						} else {
							map = (HashMap<String, String>) editorDataitemServiceImpl
						...
						break;
					case RetCodeEditor.ID:
						editorType = "响应码";
						EditorRetcodeServiceImpl editorRetcodeServiceImpl = new EditorRetcodeServiceImpl();
						if (searchByIdRadioButton.getSelection()) {
							map = (HashMap<String, String>) editorRetcodeServiceImpl
									.queryRetcodeByIdOrName(
											searchText.getText(), "",
											editor.getPs());
						} else {
							map = (HashMap<String, String>) editorRetcodeServiceImpl
				...
				if (map != null) {
					if (((EditorPart) editor).isDirty()) {
						/*int choice = editor.showMessage(SWT.ICON_INFORMATION
								| SWT.YES | SWT.NO | SWT.ICON_CANCEL, "提示",
								"是否先保存当前修改？");*/
						int choice = DevLogger.showMessage(SWT.ICON_INFORMATION
								| SWT.YES | SWT.NO | SWT.ICON_CANCEL, "提示",
								"是否先保存当前修改？");
						if (choice == SWT.YES) {
							editor.save();
					...
					// 设置该编辑器标签的标题
					editor.setEditorPartName(editorType + " " + map.get("ID")
							+ " " + "所属工程" + " " + upProject);
					// 设置工具的标题
					PlatformUI
							.getWorkbench()
							...
							.getShell()
							.setText(
									"GOLP TOOL" + " " + editorType + " "
											+ map.get("ID") + " " + "所属工程"
											+ " " + upProject);
					ResourceLeafNode node = new ResourceLeafNode(map
							.get("NAME"), map.get("ID"), editor.getThisNode()
							.getParent());
					// 更改thisNode
					editor.setThisNode(node);
					// 更改input
					editor.setInputNode(node);
					// 更新导航视图的焦点
					editor.setSelectNode(node);
					// 相当于点击了“锁定”按钮
				...
				// 若未找到，则弹出对话框提示
					...
					/*editor.showMessage(SWT.ICON_INFORMATION | SWT.YES, "抱歉",
							"未找到记录");*/
					DevLogger.showMessage(SWT.ICON_INFORMATION | SWT.YES, "抱歉",
							"未找到记录");
				}
			}
		});
