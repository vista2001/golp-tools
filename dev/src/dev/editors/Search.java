			...
			public void widgetSelected(SelectionEvent e)
			{
				...
				try
				{
					...
					case ServerEditor.ID:
						...
						if(searchByIdRadioButton.getSelection())
						{
							map = editorsServerServiceImpl.queryServerByIdOrName(searchText.getText(), 0, editor.getPs());
						}
						else
						{
							map = editorsServerServiceImpl.queryServerByIdOrName(searchText.getText(), 1, editor.getPs());
						}
						break;
					case TradeEditor.ID:
						editorType = "交易";
						...
						if(searchByIdRadioButton.getSelection())
						{
							map = (HashMap<String, String>) editorsDaoServiceImpl1.queryTradeByIdOrName(searchText.getText(), "", editor.getPs());
						}
						else
						{
							map = (HashMap<String, String>) editorsDaoServiceImpl1.queryTradeByIdOrName("", searchText.getText(), editor.getPs());
					...
					case AopDllEditor.ID:
						...
						if(searchByIdRadioButton.getSelection())
						{
							map = (HashMap<String, String>) editorAopDllServiceImpl.queryAopDllByIdOrName(searchText.getText(), "", editor.getPs());
						}
						else
						{
							map = (HashMap<String, String>) editorAopDllServiceImpl.queryAopDllByIdOrName("",searchText.getText(),editor.getPs());
					...
					case AopEditor.ID:
						...
						if(searchByIdRadioButton.getSelection())
						{
							map = (HashMap<String, String>) editorAopServiceImpl.queryAopByIdOrName(searchText.getText(), "", editor.getPs());
						}
						else
						{
							map = (HashMap<String, String>) editorAopServiceImpl.queryAopByIdOrName("",searchText.getText(),editor.getPs());
					...
					case DataItemEditor.ID:
						...
						if(searchByIdRadioButton.getSelection())
						{
							map = (HashMap<String, String>) editorDataitemServiceImpl.queryDataitemByIdOrName(searchText.getText(), "", editor.getPs());
						}
						else
						{
							map = (HashMap<String, String>) editorDataitemServiceImpl.queryDataitemByIdOrName("",searchText.getText(),editor.getPs());
					...
					case RetCodeEditor.ID:
						...
						if(searchByIdRadioButton.getSelection())
						{
							map = (HashMap<String, String>) editorRetcodeServiceImpl.queryRetcodeByIdOrName(searchText.getText(), "", editor.getPs());
						}
						else
						{
							map = (HashMap<String, String>) editorRetcodeServiceImpl.queryRetcodeByIdOrName("",searchText.getText(),editor.getPs());
...
//					editor.setSaveButtonEnable(false);
					...
					//设置该编辑器标签的标题
					editor.setEditorPartName(editorType + " " + map.get("ID") + " " + "所属工程" + " " + upProject);
					//设置工具的标题
					PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell().setText("GOLP TOOL" + " " + editorType + " " + map.get("ID") + " " + "所属工程" + " " + upProject);
					ResourceLeafNode node = new ResourceLeafNode(map.get("NAME"),map.get("ID"),editor.getThisNode().getParent());
					//更改thisNode
