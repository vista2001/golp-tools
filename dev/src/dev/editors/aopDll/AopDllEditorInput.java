...
/**
 * AopDll表编辑器的Input类
 * <p>这个类实现了IEditorInput接口，与AopDllEditor类一起完成AopDll编辑器的功能<br>
 * 在编辑器的初始化的时候，将AopDllEditor类完成编辑器的初始化所需要的数据传入，并在编辑器
 * 存在的时候保存编辑器的配置信息。
 * @see#getName()
 * @see#getToolTipText()
 * @see#setName()
...
public class AopDllEditorInput implements IEditorInput {
	...
	public boolean equals(Object obj) {
		  ...
		  if(!(obj instanceof AopDllEditorInput)) return false;
		   
		  if(!getName().equals(((AopDllEditorInput)obj).getName())) return false;
		   
		  if(!node.getRootProject().equals(((AopDllEditorInput)obj).getSource().getRootProject())) return false;
		  return true;
		 }
}
