...
public class CommonDialogServiceImpl implements CommonDialogService {
	...
	public List<TServer> serverDialogQuery(String prjId) throws SQLException {
		...
		try {
			dbConnectImpl.openConn(ps);
			rs = dbConnectImpl
					.retrive("select serverId, serverName, serverDesc"
	...
	public void updateDataItemState(String prjId, String[] published,
		...
		try {
			...
			for (String tmp : published) {
				String sql = "update dataItem set ISPUBLISHED ='1' where DATAITEMID ="
						+ Integer.parseInt(tmp);
				dbConnectImpl.executeExceptQuery(sql);
			}
			for (String tmp : nonPublished) {
				String sql = "update dataItem set ISPUBLISHED ='0' where DATAITEMID ="
						+ Integer.parseInt(tmp);
				dbConnectImpl.executeExceptQuery(sql);
			}
