...
public class RemoteDialog extends Dialog
{
    ...
    protected Control createDialogArea(Composite parent)
    {
        ...
        {
            e2.printStackTrace();
            showMessage(SWT.OK|SWT.ICON_WARNING, "失败", "连接服务器失败，请检查您的工程属性然后重试，或手动输入此内容。");
        }
    ...
    private void openMessagebox()
    {
        ...
        // box.open();
        showMessage(SWT.OK|SWT.ICON_WARNING, "失败", "没有权限打开此目录!");
    }

    public int showMessage(int style, String title, String message)
    {
        MessageBox box = new MessageBox(getShell(), style);
        box.setText(title);
