    ...
    public void createPartControl(Composite parent)
    {
        ...
        viewer.addSelectionChangedListener(new ISelectionChangedListener()
        {
            ...
            public void selectionChanged(SelectionChangedEvent event)
            {
                ...
                {
                    ResourceLeafNode resourceLeafNode = (ResourceLeafNode) s.getFirstElement();
                    ...
                    if (editorInput != null)
                    {
                        IEditorPart  editorPart = getSite()
                                .getWorkbenchWindow().getActivePage()
                                .findEditor(editorInput);
                        if(editorPart != null)
                        {
                            getSite().getWorkbenchWindow().getActivePage().bringToTop(editorPart);
                            getSite().getShell().setText("GOLP TOOL" + " " + editorPart.getTitle());
                        }
                    }
                }
            }
...
//        menuMgr.add(new PrjPropertyAction(getSite().getWorkbenchWindow()));
        ...
        doubleClickAction = new Action()
        {
            public void run()
            {
                ...
                if (  obj instanceof ResourceNode 
                   && !obj.getClass().getSimpleName().equals("ProjectNode"))
                {
    ...
    private void createTreeLeafs(ResourceNode obj)
    {
        ...
        try
        {
            map = navViewDaoServiceImpl
                    .getResourceNodeChild(ps, tableName);
            if (!map.isEmpty())
            {
                obj.removeAllChildren();
                for (String key : map.keySet())
                {
                    ResourceLeafNode rln = new ResourceLeafNode(map.get(key), key,
                            obj);
