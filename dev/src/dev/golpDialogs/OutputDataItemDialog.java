...
public class OutputDataItemDialog extends TitleAreaDialog {
	...
	public OutputDataItemDialog(Shell parentShell, Object obj,
		...
		for (String s : tmp) {
			...
			DevLogger.printDebugMsg(s.substring(0, s.indexOf('@')));
		}
	}
	...
	protected Control createDialogArea(Composite parent) {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (allDataItemList.getSelectionCount() > 0) {
					...
					if (allDataItemList.getSelectionCount() == 1) {
						dataItemInfoText.setText(dataItemInfo[allDataItemList
								.getSelectionIndex()]);
					} else {
				...
				if (fromReqCombo.getSelectionIndex() != -1) {
					if (outputDataItemList.getSelectionCount() > 0) {
						...
						for (int i : outputDataItemList.getSelectionIndices()) {
							...
							DevLogger.printDebugMsg(fromReqCombo
									.getSelectionIndex());
							if (fromReqCombo.getSelectionIndex() == 0) {
								...
								if (!inputDatas.contains(selected)) {
									...
									messageBox.setMessage(selected
											+ " 未在输入数据项中配置,因此来源不能为输入数据!");
									messageBox.open();*/
									DevLogger.showMessage(SWT.ICON_ERROR, "未配置数据项", selected
											+ " 未在输入数据项中配置,因此来源不能为输入数据!");
								}
							} else {
								outputDataItemList.setItem(
										i,
										sub
												+ (fromReqCombo
														.getSelectionIndex() + 1));
		...
		this.setMessage("请选择输出数据项，并为每一项选择是否必须和是否来自请求数据。"
				+ System.getProperty("line.separator") + "注意：重复的数据项将不会被添加！");
