...
public class QueryTradeDialog extends TitleAreaDialog {
	 ...
	 *            要查询的交易所属的工程
	 * @param selfId
	 *            在查询到的结果中要排除的交易Id（即前述a），若selfId为-1，即表示不排除任何交易Id，显示所有交易的Id。
	 */
		...
		listInDialog.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if ((listInDialog.getSelectionCount() == 1)
					...
					tradeInfoText.setText(tradeInfo[listInDialog
							.getSelectionIndex()]);
				} else {
	...
	private void initData(String prjId) {
		...
		try {
			java.util.List<TTrade> trades = commonDialogServiceImpl
					.tradeDialogQuery(prjId, type);
