...
public class QueryServerDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		serverInfoText = new Text(container, SWT.BORDER | SWT.READ_ONLY
				| SWT.WRAP | SWT.V_SCROLL | SWT.MULTI);
		serverInfoText.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				false, 1, 1));
		listInDialog.addSelectionListener(new SelectionAdapter() {

			@Override
			public void widgetSelected(SelectionEvent e) {
				serverInfoText.setText(serverInfo[listInDialog
						.getSelectionIndex()]);
			}
	...
	private void initData(String prjId) {
		...
		try {
			java.util.List<TServer> servers = commonDialogServiceImpl
					.serverDialogQuery(prjId);
