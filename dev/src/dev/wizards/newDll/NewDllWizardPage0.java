...
public class NewDllWizardPage0 extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		aopDllUpProjectCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				true, false, 1, 1));
		...
		aopDllLevelCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		...
		aopDllTypeCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		...
		aopDllIdText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		new Label(container, SWT.NONE);
		...
		aopDllNameText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		new Label(container, SWT.NONE);
		...
		aopDllPathButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				RemoteDialog remoteDialog = new RemoteDialog(getShell(),
						aopDllUpProjectCombo.getText(), null,
						RemoteDialog.REMOTEDIALOG_FILE,
		...
		aopDllDescText = new Text(container, SWT.BORDER | SWT.WRAP
		...
		aopDllUpProjectCombo.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				...
				if (aopDllUpProjectCombo.getText().isEmpty() == false) {
					setDllIdText(CommonUtil.initPs(aopDllUpProjectCombo
							.getText()));
	 ...
	 * 所新建的动态库的ID，若AOPDLL表为空，则查找project的AOPDLLIDSTART 字段，此值即为所新建的动态库的ID。
	 * 
	 * @param ps
	 */
	private void setDllIdText(PreferenceStore ps) {
		...
		try {
			TAopDll aopDll = commonDialogServiceImpl
					.getNewAopDllId(aopDllUpProjectCombo.getText());
