/* 文件名：       NewAopWizardPage0.java
 * 修改人：       rxy
 * 修改时间：   2013.12.13
 * 修改内容：    增加处理：当用户选择了某一所属工程后，读取该工程的数据库中的动态库记录，
 *         显示在NewAopWizardPage1的upDllCombo中。
 */
	...
	public void createControl(Composite parent)
	{
		...
		upProjectCombo.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				...
				{
				    ((NewAopWizardPage1)getWizard().getPage("NewAopWizardPage1")).initData(upProjectCombo.getText());
				    ((NewAopWizardPage1)getWizard().getPage("NewAopWizardPage1")).getInputData().setText("");
				    ((NewAopWizardPage1)getWizard().getPage("NewAopWizardPage1")).getOutPutData().setText("");
				}
			}
		});
		...
		aopLvlCombo.setItems(new String[]{"0-APP"});
		...
		aopDescText = new Text(container, SWT.BORDER | SWT.WRAP | SWT.V_SCROLL | SWT.MULTI);
