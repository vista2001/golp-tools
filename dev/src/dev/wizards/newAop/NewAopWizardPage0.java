...
public class NewAopWizardPage0 extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		upProjectCombo.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				...
				if (!upProjectCombo.getText().isEmpty()) {
					((NewAopWizardPage1) getWizard().getPage(
							"NewAopWizardPage1")).initData(upProjectCombo
							.getText());
					((NewAopWizardPage1) getWizard().getPage(
							"NewAopWizardPage1")).getInputData().setText("");
					((NewAopWizardPage1) getWizard().getPage(
							"NewAopWizardPage1")).getOutPutData().setText("");
				}
			}
		});
		...
		aopLvlCombo.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		...
		aopDescText = new Text(container, SWT.BORDER | SWT.WRAP | SWT.V_SCROLL
				| SWT.MULTI);
