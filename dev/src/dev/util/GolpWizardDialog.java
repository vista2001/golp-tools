/* 文件名：       GolpWizardDialog.java
 * 描述：           该文件定义了类GolpWizardDialog，该类对org.eclipse.jface.wizard.WizardDialog类
 *         做了细微的改动，目的在于取消向导对话框自动将“完成”按钮设为默认按钮。
 *         修改1：将createButtonsForButtonBar方法中，为finishButton赋值时，所调用的createButton
 *         方法的最后一个参数由true变为false；
 *         修改2：将updateButtons方法最后的if-else块注释掉。
 * 创建人：       rxy
 * 创建时间：   2013.12.28
 */
...
/**
 * 该类对org.eclipse.jface.wizard.WizardDialog类 做了细微的改动，目的在于取消向导对话框自动将“完成”按钮设为默认按钮。
 */
public class GolpWizardDialog extends TitleAreaDialog implements
	...
	/**
	 * A delay in milliseconds that reduces the risk that the user accidentally
	 * triggers a button by pressing the 'Enter' key immediately after a job has
	 * finished.
	 * 
	 * @since 3.6
	 */
	 ...
	 */
	public GolpWizardDialog(Shell parentShell, IWizard newWizard) {
		super(parentShell);
	...
	private Object aboutToStart(boolean enableCancelButton) {
		...
		if (getShell() != null) {
			...
			// Install traverse listener once in order to implement 'Enter' and
			// 'Space' key blocking
				...
				getShell().addTraverseListener(new TraverseListener() {
					public void keyTraversed(TraverseEvent e) {
						if (e.detail == SWT.TRAVERSE_RETURN
							...
							// We want to ignore the keystroke when we detect
							// that it has been received within the
							// delay period after the last operation has
							// finished. This prevents the user from
							// accidentally
							// hitting "Enter" or "Space", intending to cancel
							// an operation, but having it processed exactly
							// when the operation finished, thus traversing the
							// wizard. If there is another operation still
							// running, the UI is locked anyway so we are not in
							// this code. This listener should fire only
							// after the UI state is restored (which by
							// definition means all jobs are done.
							// See
							// https://bugs.eclipse.org/bugs/show_bug.cgi?id=287887
							if (timeWhenLastJobFinished != 0
									&& System.currentTimeMillis()
											- timeWhenLastJobFinished < RESTORE_ENTER_DELAY) {
								e.doit = false;
								return;
							}
							timeWhenLastJobFinished = 0;
	...
	protected void createButtonsForButtonBar(Composite parent) {
		...
		// 修改1：将下面这条语句的最后一个参数有true修改为false。
	...
	protected Control createDialogArea(Composite parent) {
		...
		progressMonitorPart = createProgressMonitorPart(composite,
				new GridLayout());
	...
	private Composite createPreviousAndNextButtons(Composite parent) {
		...
		final String backReaderText = IDialogConstants.BACK_LABEL.replace('<',
				' ');
		backButton.getAccessible().addAccessibleListener(
		...
		final String nextReaderText = IDialogConstants.NEXT_LABEL.replace('>',
				' ');
		nextButton.getAccessible().addAccessibleListener(
				new AccessibleAdapter() {
					public void getName(AccessibleEvent e) {
